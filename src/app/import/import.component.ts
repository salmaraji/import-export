...
export class ImportComponent {
  ...
  onFileChange(event: Event) {
    ...
    if (input.files && input.files.length > 0) {
      ...
      if (extension === 'xlsx') {
        ...
        alert(`Le fichier ${file.name} a une extension invalide. Seuls les fichiers .xlsx sont acceptés.`);
        input.value = '';
      }
    }
  }
  ...
  importData() {
    ...
    reader.onload = (e: ProgressEvent<FileReader>) => {
      const tabstr: string | ArrayBuffer = e.target?.result as string | ArrayBuffer;
      const wb: XLSX.WorkBook = XLSX.read(tabstr, { type: 'binary' });
      const wsname: string = wb.SheetNames[0];
      const ws: XLSX.WorkSheet = wb.Sheets[wsname];
      this.importedData = XLSX.utils.sheet_to_json(ws, { header: 1 }) as any[][];
      ...
      if (this.importedData.length > 0) {
        ...
        this.dataImported.emit({ columns: this.columns, importedData: this.importedData });
      }
    };
